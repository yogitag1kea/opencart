<?php
class ModelUserRegister extends Model {
	...
	// }
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "seller` SET user_group_id = '" . (int)$user_group_id . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', pancard = '" . $this->db->escape($data['pancard']) . "', date_added = NOW()");
