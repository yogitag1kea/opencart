...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		$this->cache->delete('seller.language');
	...
	public function editLanguage($language_id, $data) {
		...
		if ($language_query->row['code'] != $data['code']) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "setting SET value = '" . $this->db->escape($data['code']) . "' WHERE `key` = 'config_seller_language' AND value = '" . $this->db->escape($language_query->row['code']) . "'");
		}
	...
	public function getLanguages($data = array()) {
		if ($data) {
			...
			if (!$language_data) {
				...
				$this->cache->set('seller.language', $language_data);
			}
