...
class ControllerMarketplaceInstall extends Controller {
	public function install() {
		...
		if (!$this->seller->hasPermission('modify', 'marketplace/install')) {
	...
	public function move() {
		...
		if (!$json) {
			...
			if (is_dir($directory . 'upload/')) {
				...
				$allowed = array(
					'seller/controller/extension/',
					'seller/language/',
					'seller/model/extension/',
					'seller/view/image/',
					'seller/view/javascript/',
					'seller/view/stylesheet/',
					'seller/view/template/extension/',
				...
				foreach ($files as $file) {
					...
					if ($safe) {
						...
						if (substr($destination, 0, 5) == 'seller') {
	...
	public function uninstall() {
		...
		if (!$json) {
			...
			foreach ($results as $result) {
				...
				// Check if the copy location exists or not
				if (substr($result['path'], 0, 5) == 'seller') {
