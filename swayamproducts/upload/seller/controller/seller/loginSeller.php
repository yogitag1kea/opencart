<?php
class ControllerSellerLoginSeller extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('common/sellerLogin');
		...
		// $data['action'] = $this->url->link('seller/sellerLogin', '', true);
		...
		// $this->response->setOutput($this->load->view('seller/loginSeller', $data));
		return $this->load->view('seller/loginSeller', $data);
	}

	protected function validate() {
		if (!isset($this->request->post['username']) || !isset($this->request->post['password']) || !$this->seller->login($this->request->post['username'], html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8'))) {
