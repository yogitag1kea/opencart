			...
			if ($response) {
				...
				if ($template) {
					...
					if ($json_decoded) {
						...
						$log->write("seller/openbay/amazon_product/parseTemplateAjax failed to parse template response: " . $response);
					}
				}
			} else {
				$log->write("seller/openbay/amazonus_product/parseTemplateAjax failed calling productv2/GetTemplateXml with params: " . print_r($request, true));
			}
		}
