...
class ControllerExtensionOpenbayFba extends Controller {
    public function install() {
        ...
        $this->model_user_user_group->addPermission($this->seller->getGroupId(), 'access', 'extension/openbay/fba');
        $this->model_user_user_group->addPermission($this->seller->getGroupId(), 'modify', 'extension/openbay/fba');
    ...
    protected function validate() {
        if (!$this->seller->hasPermission('modify', 'extension/openbay/fba')) {
    ...
    public function shipFulfillment() {
        ...
        if (empty($this->request->get['order_id']) || empty($this->request->get['fba_order_fulfillment_id'])) {
            ...
            if (!isset($response['response_http']) || $response['response_http'] != 200) {
                /**
                 * @todo notify the seller about any errors
                 */
