...
class ModelSellerUser extends Model {
	...
	print_r($selectSellerId);
	echo ($user_group_id);
	$this->db->query("INSERT INTO `" . DB_PREFIX . "seller` SET username = '" . $this->db->escape($data['username']) . "', user_group_id = '" . (int)$user_group_id . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "',  pancard = '" . $this->db->escape($data['pancard']) . "', image = '" . $this->db->escape($data['image']) . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
	...
	public function editUser($user_id, $data) {
		$selectSellerId = $this->db->query("SELECT user_group_id FROM " . DB_PREFIX . "user_group WHERE name = 'seller'" );
		$user_group_id =  $selectSellerId->row['user_group_id'];
		$this->db->query("UPDATE `" . DB_PREFIX . "seller` SET username = '" . $this->db->escape($data['username']) . "', user_group_id = '" . (int)$user_group_id . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', image = '" . $this->db->escape($data['image']) . "', status = '" . (int)$data['status'] . "' WHERE user_id = '" . (int)$user_id . "'");
	...
	public function getUser($user_id) {
		$query = $this->db->query("SELECT *, (SELECT ug.name FROM `" . DB_PREFIX . "user_group` ug WHERE ug.user_group_id = u.user_group_id) AS user_group FROM `" . DB_PREFIX . "seller` u WHERE u.user_id = '" . (int)$user_id . "'");
