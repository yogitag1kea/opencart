...
class ControllerSellerUser extends Controller {
	...
	protected function getList() {
		...
		$user_total = $this->model_seller_user->getTotalUsers();
	...
	protected function getForm() {
		...
		if (isset($this->request->post['pancard'])) {
			...
			$data['pancard'] = $user_info['pancard'];
		} else {
			$data['pancard'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['email']) < 3) || (utf8_strlen($this->request->post['email']) > 30)) {
			$this->error['email'] = $this->language->get('error_username');
		}
		...
		$user_info = $this->model_seller_user->getUserByUsername($this->request->post['email']);
		...
		if ((utf8_strlen(trim($this->request->post['pancard'])) < 10) || (utf8_strlen(trim($this->request->post['pancard'])) > 10)) {
			$this->error['pancard'] = $this->language->get('error_pancard');
		}
