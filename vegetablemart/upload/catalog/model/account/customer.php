...
class ModelAccountCustomer extends Model {
	public function addCustomer($data, $otp) {
		...
		$customer = $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape($otp) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', is_customer = 0, date_added = NOW()");
	...
	public function getCustomerByOtp($mobileNo, $otp) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE telephone = '" . (int)$mobileNo . "' and  password = '" . (int)$otp . "'");

		return $query->row;
		
	}

	public function updateCustomerByOtp($mobileNo, $otp){
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET is_customer = '1' WHERE telephone = '" . (int)$mobileNo . "' and  password = '" . (int)$otp . "'");
	}
	...
	public function getCustomerByEmail($telephone) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($telephone)) . "'");
	...
	public function getTotalCustomersByEmail($telephone) {
	...
	public function addLoginAttempt($telephone) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_login WHERE telephone = '" . $this->db->escape($telephone) . "' AND ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "'");

		if (!$query->num_rows) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_login SET telephone = $telephone, ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', total = 1, date_added = '" . $this->db->escape(date('Y-m-d H:i:s')) . "', date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "'");
		} else {
	...
	public function getLoginAttempts($telephone) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_login` WHERE telephone = $telephone");
		return $query->row;
	}

	public function deleteLoginAttempts($telephone) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_login` WHERE telephone = $telephone");
	}
